<?php
$pageTitle = 'Modifier la Discussion - Forum AlgoCodeBF';
...
// Utiliser les valeurs du post si pas d'anciennes valeurs
if (empty($old) && isset($post)) {
    $old = $post;
}
?>
    ...
    <div class="container">
        <div class="create-post-container">
            <div class="page-header">
                <h1><i class="fas fa-edit"></i> Modifier la Discussion</h1>
                <p>Modifiez votre discussion et mettez à jour son contenu</p>
            </div>
            ...
            <form action="<?= BASE_URL ?>/forum/edit/<?= $post['id'] ?>" method="POST" class="create-post-form" enctype="multipart/form-data">
                ...
                <?php if (!empty($existingAttachments)): ?>
                    <div class="form-group">
                        <label>
                            <i class="fas fa-paperclip"></i> Pièces jointes actuelles
                        </label>
                        <div class="existing-attachments">
                            <?php foreach ($existingAttachments as $attachment): ?>
                                <div class="existing-attachment-item" id="attachment-<?= $attachment['id'] ?>">
                                    <div class="attachment-icon-small">
                                        <?php
                                        $icon = 'fa-file';
                                        if (strpos($attachment['mime_type'], 'pdf') !== false) $icon = 'fa-file-pdf';
                                        elseif (strpos($attachment['mime_type'], 'image') !== false) $icon = 'fa-file-image';
                                        elseif (strpos($attachment['mime_type'], 'word') !== false) $icon = 'fa-file-word';
                                        elseif (strpos($attachment['mime_type'], 'zip') !== false) $icon = 'fa-file-archive';
                                        ?>
                                        <i class="fas <?= $icon ?>"></i>
                                    </div>
                                    <div class="attachment-name-small">
                                        <?= htmlspecialchars($attachment['original_name']) ?>
                                        <small><?= round($attachment['file_size'] / 1024, 2) ?> KB</small>
                                    </div>
                                    <button type="button" 
                ...
                <div class="form-group">
                    <label for="attachments">
                        <i class="fas fa-cloud-upload-alt"></i> Ajouter de nouvelles pièces jointes (optionnel)
                    </label>
                ...
                <div class="form-actions">
                    ...
                    <a href="<?= BASE_URL ?>/forum/show/<?= $post['id'] ?>" class="btn btn-secondary btn-lg">
...
function removeAttachment(attachmentId) {
    if (confirm('Êtes-vous sûr de vouloir supprimer cette pièce jointe ?')) {
        fetch('<?= BASE_URL ?>/forum/deleteAttachment/' + attachmentId, {
            method: 'POST',
            headers: {
